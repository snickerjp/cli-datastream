# DataStream CLI HTTP Calls Module
"""
HTTP communication utilities for Akamai DataStream CLI.

Originally based on Akamai EdgeGrid Python module.
...
class EdgeGridHttpCaller:
    ...
    def getResult(self, endpoint, parameters=None):
        ...
        path = endpoint
        endpoint_result = self.session.get(
            parse.urljoin(self.baseurl, path), params=parameters
        )
        if self.verbose:
    ...
    def httpErrors(self, status_code, endpoint, result):
        ...
        if status_code in [404]:
            ...
            error_msg += (
                "ERROR: This means that the object does not exist as requested.\n"
            )
        ...
        if "errorString" in result:
            ...
            for key in result:
                if (
                    ...
                    continue
                if (
                    "errorString" in result[key]
                    and type(result[key]["errorString"]) is str
                ):
                    error_string = result[key]["errorString"]
        if error_string:
            error_msg = "ERROR: Call caused a server fault.\n"
            error_msg += (
                "ERROR: Please check the problem details for more information:\n"
            )
            error_msg += "ERROR: Problem details: %s\n" % error_string
            exit(error_msg)
    ...
    def postResult(self, endpoint, body, parameters=None):
        ...
        path = endpoint
        endpoint_result = self.session.post(
            parse.urljoin(self.baseurl, path),
            data=body,
            headers=headers,
            params=parameters,
        )
        ...
        if self.verbose:
            print(
                ...
                % (
                    ...
                    parameters,
                    status,
                    endpoint_result.headers["content-type"],
                )
            )
        if status == 204:
            return {}
    ...
    def postFiles(self, endpoint, file, parameters=None):
        ...
        path = endpoint
        endpoint_result = self.session.post(
            parse.urljoin(self.baseurl, path), files=file, params=parameters
        )
        status = endpoint_result.status_code
        if self.verbose:
            print(
                "LOG: POST FILES %s %s %s"
    ...
    def putResult(self, endpoint, body, parameters=None):
        ...
        endpoint_result = self.session.put(
            parse.urljoin(self.baseurl, path),
            data=body,
            headers=headers,
            params=parameters,
    ...
    def deleteResult(self, endpoint, parameters=None):
        ...
        endpoint_result = self.session.delete(
            parse.urljoin(self.baseurl, endpoint), params=parameters
        )
        status = endpoint_result.status_code
        if self.verbose:
            print(
