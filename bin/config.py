...
class EdgeGridConfig:
    ...
    def __init__(self, config_values, configuration, flags=None):
        parser = self.parser
        parser.add_argument(
            "--verbose", "-v", default=False, action="count", help=" Verbose mode"
        )
        parser.add_argument(
            "--debug",
            "-d",
        ...
        parser.add_argument(
            "--section",
            "-c",
            default="default",  # Changed from "ds" to "default" for standard EdgeRC compatibility
            metavar="credentials_file_section",
            action="store",
        ...
        parser.add_argument(
            "--accountSwitchKey",
            "-a",
            metavar="Account Switch Key",
            action="store",
            help=" Switch key to different account",
        )

        subparsers = parser.add_subparsers(help="commands", dest="command")

        list_group_parser = subparsers.add_parser(
            "list-groups", help="List all Groups in the Account"
        )
        list_group_parser.add_argument(
            ...
            choices=["json", "text"],
            metavar="json/text",
            help=" Output type {json, text}. Default is text",
        )

        list_connectors_parser = subparsers.add_parser(
            "list-connectors", help="List all Connectors."
        )
        list_connectors_parser.add_argument(
            "--output-type",
            "-t",
            default="text",
            choices=["json", "text"],
        ...
        list_stream_type_parser = subparsers.add_parser(
            "list-stream-types", help="List all Types of Stream."
        )
        list_stream_type_parser.add_argument(
            "--output-type",
            "-t",
            default="text",
            choices=["json", "text"],
        ...
        list_streams_parser = subparsers.add_parser(
            "list-streams", help="List all Streams."
        )
        list_streams_parser.add_argument(
            "groupid",
            help="Group id for which streams need to be retrieve",
            action="store",
        )
        list_streams_parser.add_argument(
            "--streamstatus",
            "-s",
            default="ACTIVATED",
            choices=["ACTIVATED", "DEACTIVATED", "IN_PROGRESS"],
            metavar="activated/deactivated/in_progress",
            help=" Status of the stream. Default is Activated",
        )
        list_streams_parser.add_argument(
            "--output-type",
            "-t",
        ...
        list_properties_parser = subparsers.add_parser(
            "list-properties", help="List all Properties."
        )
        list_properties_parser.add_argument(
            "groupid",
            help="Group id for which properties need to be listed",
            action="store",
        )
        list_properties_parser.add_argument(
            "productId",
            help="Product id for which properties need to be listed",
            action="store",
        )
        list_properties_parser.add_argument(
            "--output-type",
        ...
        list_datasets_parser = subparsers.add_parser(
            "list-datasets", help="List all Datasets."
        )
        list_datasets_parser.add_argument(
            "--template",
            default="EDGE_LOGS",
            choices=["EDGE_LOGS"],
            help=" Template name for which datasets need to be fetched.[Example: EDGE_LOGS]",
        )
        list_datasets_parser.add_argument(
            "--output-type",
            "-t",
            ...
            choices=["json", "text"],
            metavar="json/text",
            help=" Output type is json or text. Default is text",
        )

        list_products_parser = subparsers.add_parser(
            "list-products", help="List all Products."
        )
        list_products_parser.add_argument(
            "--output-type",
            "-t",
            default="text",
            choices=["json", "text"],
        ...
        get_stream_parser = subparsers.add_parser(
            "get-stream", help="Get Details of Stream."
        )
        get_stream_parser.add_argument(
            "streamid", help="Details of the stream", action="store"
        )
        get_stream_parser.add_argument(
            "--version",
            "-v",
        ...
        activation_history_parser = subparsers.add_parser(
            "activation-history", help="Get Details of Stream Activation History."
        )
        activation_history_parser.add_argument(
            "streamid", help="Stream ID", action="store"
        )
        activation_history_parser.add_argument(
            "--output-type",
            "-t",
        ...
        stream_history_parser = subparsers.add_parser(
            "stream-history", help="Get Details of Stream History."
        )
        stream_history_parser.add_argument("streamid", help="Stream ID", action="store")
        stream_history_parser.add_argument(
            "--output-type",
            "-t",
            default="text",
            choices=["json", "text"],
        ...
        create_stream_parser = subparsers.add_parser(
            "create", help="Create a Particular Stream."
        )
        create_stream_parser.add_argument(
            "file",
            help="Json File consiting of Stream Details",
            type=argparse.FileType("r"),
        )

        update_stream_parser = subparsers.add_parser(
            "update", help="Update a Particular Stream."
        )
        update_stream_parser.add_argument("streamid", help="Stream ID", action="store")
        update_stream_parser.add_argument(
            "file",
            help="Json File consiting of Stream Details",
            type=argparse.FileType("r"),
        )
        ...
        for argument in config_values:
            if config_values[argument]:
                if (
                    config_values[argument] == "False"
                    or config_values[argument] == "True"
                ):
        ...
        if os.path.isfile(arguments["edgerc"]):
            ...
            for key, value in config.items(configuration):
                ...
                if key not in arguments or arguments[key] is None:
                    ...
                    print(
                        "Missing configuration file.  Run python gen_edgerc.py to get your credentials file set up once you've provisioned credentials in LUNA."
                    )
                    return None
